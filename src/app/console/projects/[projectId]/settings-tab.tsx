...
export function SettingsTab({
  ...
  const maskedSecret = webhookSecret ? `${webhookSecret.slice(0, 8)}${'*'.repeat(24)}${webhookSecret.slice(-8)}` : ''
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div className="space-y-2">
                ...
                <p className="max-w-2xl text-[13px] leading-5 text-muted-foreground/85">
                  Used to verify incoming repository webhook payloads for this project.
                </p>
              </div>
            ...
            {canManageWebhookSecret ? (
              <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center">
                <code className="min-h-10 flex-1 break-all rounded-lg border border-border/70 bg-background/70 px-3 py-2 font-mono text-[12px] leading-6 text-muted-foreground">
                  {isSecretLoading ? 'Loading webhook secret...' : displayedSecret || 'No webhook secret configured'}
                </code>
                ...
                <TooltipProvider>
                  <div className="grid grid-cols-3 gap-2 sm:flex sm:shrink-0">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>{isSecretVisible ? 'Hide secret' : 'Reveal secret'}</TooltipContent>
                    </Tooltip>
              ...
              <div className="mt-5 flex items-start gap-3 rounded-xl border border-border/70 bg-background/55 p-4 text-[13px] leading-5 text-muted-foreground">
                ...
                <p>Webhook secret controls are available to project owners, editors, and system admins.</p>
              </div>
            )}
          </div>
        </CardContent>
      ...
      <AlertDialog open={isRotateDialogOpen} onOpenChange={setIsRotateDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Existing webhook deliveries signed with the current secret will fail after this change. Update the
              repository webhook configuration with the new secret after rolling it.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
